defmodule ShakhaNowWeb.UserAuthTest do
  use ShakhaNowWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias ShakhaNow.Accounts
  alias ShakhaNow.Accounts.Scope
  alias ShakhaNowWeb.UserAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ShakhaNowWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "keeps session when re-authenticating", %{conn: conn, user: user} do
      ...
      assert get_session(conn, :to_be_kept)
      assert redirected_to(conn) == ~p"/dashboard"
    end
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ShakhaNowWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ShakhaNowWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
